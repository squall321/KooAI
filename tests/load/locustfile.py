...
class SimulationAPIUser(FastHttpUser):
            ...
            if response.status_code == 200:
                try:
                    ...
                    if data and len(data) > 0:
                        self.simulation_ids = [
                            s.get("simulation_id") for s in data if s.get("simulation_id")
                        ]
                    response.success()
    ...
    def upload_simulation(self) -> None:
        ...
        files = {"file": ("test_simulation.csv", csv_data, "text/csv")}
        data = {"name": f"Load Test Simulation {self.uploaded_count}", "auto_analyze": "false"}
    ...
    def convergence_analysis(self) -> None:
        ...
        payload = {"field_name": "temperature", "window_size": random.randint(3, 10)}
    ...
    def spatial_analysis(self) -> None:
        ...
        payload = {"field_name": "temperature", "min_value": 300.0, "max_value": 500.0}
...
class ReadOnlyUser(FastHttpUser):
    ...
    def analyze_field(self) -> None:
        ...
        if self.simulation_ids:
            sim_id = random.choice(self.simulation_ids)
            self.client.post(
                f"/api/v1/simulations/{sim_id}/analyze", json={"field_name": "temperature"}
            )
...
class WriteHeavyUser(FastHttpUser):
    ...
    def upload_simulation(self) -> None:
        ...
        response = self.client.post("/api/v1/simulations/upload", files=files, data=data)
