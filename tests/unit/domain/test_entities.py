...
class TestSimulationResult:
    ...
    def test_create_with_empty_type_raises_error(self):
        """빈 타입으로 생성 시 에러"""
        with pytest.raises(ValueError, match="type cannot be empty"):
            SimulationResult(name="Test", type="")

    def test_create_with_too_long_name_raises_error(self):
        """너무 긴 이름으로 생성 시 에러"""
        long_name = "x" * 256
...
class TestDataset:
    ...
    def test_create_with_empty_data_type_raises_error(self):
        """빈 데이터 타입으로 생성 시 에러"""
        from uuid import uuid4
...
class TestAnalysis:
    ...
    def test_start_running_analysis_raises_error(self):
        """이미 실행 중인 분석 시작 시 에러"""
        from uuid import uuid4
    ...
    def test_complete_pending_analysis_raises_error(self):
        """대기 중인 분석 완료 시 에러"""
        from uuid import uuid4
