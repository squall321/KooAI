...
class TestCLICommands:
    ...
    def test_upload_command(self, runner: CliRunner, sample_csv_file: Path) -> None:
        """업로드 명령어"""
        result = runner.invoke(cli, ["upload", str(sample_csv_file)])
        assert result.exit_code == 0
        assert "Uploaded" in result.output or "success" in result.output.lower()

    def test_upload_nonexistent_file(self, runner: CliRunner) -> None:
        """존재하지 않는 파일 업로드"""
        result = runner.invoke(cli, ["upload", "nonexistent.csv"])
        assert result.exit_code != 0

    def test_list_command(self, runner: CliRunner, sample_csv_file: Path) -> None:
        """목록 명령어"""
        # 먼저 업로드
    ...
    def test_upload_and_info(self, runner: CliRunner, sample_csv_file: Path) -> None:
        """업로드 후 정보 조회"""
        # 업로드
    ...
    def test_convergence_command_help(self, runner: CliRunner) -> None:
        """수렴성 명령어 헬프"""
        result = runner.invoke(cli, ["convergence", "--help"])
        assert result.exit_code == 0

    def test_spatial_command_help(self, runner: CliRunner) -> None:
        """공간 분석 명령어 헬프"""
