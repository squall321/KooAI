...
class DifferenceAnalyzer:
    ...
    def generate_summary(self, differences: Dict[str, FieldDifference]) -> Dict:
        """
        ...
        sorted_fields = sorted(
            differences.items(), key=lambda x: x[1].mean_relative_diff, reverse=True
        )
    ...
    def _assess_overall_status(self, differences: Dict[str, FieldDifference]) -> str:
        """Assess overall status of comparison"""
        critical_count = sum(
            1 for diff in differences.values() if diff.difference_type == DifferenceType.CRITICAL
        )
        large_count = sum(
            1 for diff in differences.values() if diff.difference_type == DifferenceType.LARGE
        )
